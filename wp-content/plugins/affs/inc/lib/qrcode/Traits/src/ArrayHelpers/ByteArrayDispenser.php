...
class ByteArrayDispenser {
	...
	public function guessFrom( $data ): ByteArray {
		...
		if (is_string($data)) {
			...
			foreach (array( 'Bin', 'Hex', 'JSON', 'Base64' ) as $type) {

				if (call_user_func_array(array( $this, 'isAllowed' . $type ), array( $data )) === true) {
					return call_user_func_array(array( $this, 'from' . $type ), array( $data ));
				}
